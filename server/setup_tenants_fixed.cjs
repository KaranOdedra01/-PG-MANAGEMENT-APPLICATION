...
const tenantModel = `import mongoose from 'mongoose';
...
const tenantController = `import mongoose from 'mongoose';
...
const tenantRoutes = `import express from 'express';
import {
  getTenants,
  getTenantById,
  onboardTenant,
...
fs.writeFileSync(path.join(__dirname, 'src/models/Tenant.js'), tenantModel, 'utf8');
fs.writeFileSync(path.join(__dirname, 'src/controllers/tenantController.js'), tenantController, 'utf8');
fs.writeFileSync(path.join(__dirname, 'src/routes/tenantRoutes.js'), tenantRoutes, 'utf8');
console.log('Successfully generated Tenant backend files!');
